...
class BrowserActionResult(BaseModel):
    ...
    interactive_elements: Optional[
        List[Dict[str, Any]]
    ] = None  # Simplified list of interactive elements
...
class BrowserAutomation:
    ...
    def get_configure_router(self) -> APIRouter:
        @asynccontextmanager
        ...
        self.router = APIRouter(lifespan=router_lifespan)
    ...
    async def navigate_to(self, action: GoToUrlAction = Body(...)):
        ...
        try:
            print(f"Navigating to URL {action.url}")
            page = await self.get_current_page()
            ...
            try:
                ...
                return self.build_action_result(
                    False, str(e), None, "", "", {}, error=str(e), content=None
                )
    ...
    async def scroll_to_text(self, text: str = Body(...)):
        ...
        try:
            ...
            message = (
                f"Scrolled to text: {text}"
                if found
                else f"Text '{text}' not found or not visible on page"
            )

            return self.build_action_result(
                found, message, dom_state, screenshot, elements, metadata, error="", content=None
            )
        except Exception as e:
            return self.build_action_result(
                False, str(e), None, "", "", {}, error=str(e), content=None
        ...
        try:
            ...
            try:
                if element.tag_name.lower() == "select":
                    ...
                    await page.click(f"#{element.attributes.get('id')}") if element.attributes.get(
                        "id"
                    ) else None
    ...
    async def drag_drop(self, action: DragDropAction = Body(...)):
        ...
        try:
            ...
            return self.build_action_result(
                True, message, dom_state, screenshot, elements, metadata, error="", content=None
            )
        except Exception as e:
            return self.build_action_result(
                False, str(e), None, "", "", {}, error=str(e), content=None
...
api_app.include_router(automation_service.get_configure_router(), prefix="/api")
...
async def test_browser_api():
    ...
    try:
        ...
        print("\n--- Testing Search ---")
        result = await automation_service.search_google(
            SearchGoogleAction(query="browser automation")
        )
        ...
        coord_click_result = await automation_service.click_coordinates(
            ClickCoordinatesAction(x=100, y=100)
        )
        print(
            f"Coordinate click status: {'✅ Success' if coord_click_result.success else '❌ Failed'}"
        )
        ...
        if content_result.content:
            content_preview = (
                content_result.content[:100] + "..."
                if len(content_result.content) > 100
                else content_result.content
            )
    ...
    try:
        ...
        if result.element_count > 4:  # Ensure element 5 exists
            ...
            click_result = await automation_service.click_element(
                ClickElementAction(index=click_index)
            )
            print(
                f"Click status for element {click_index}: {'✅ Success' if click_result.success else '❌ Failed'}"
            )
            print(f"Message: {click_result.message}")
            ...
            if click_result.element_count > 0:  # Check if there are still elements
                click_index_2 = 1
                click_result_2 = await automation_service.click_element(
                    ClickElementAction(index=click_index_2)
                )
                print(
                ...
                if (
                    click_result_2.interactive_elements
                    and len(click_result_2.interactive_elements) > 0
                ):
